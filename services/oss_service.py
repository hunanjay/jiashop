...
def upload_base64_to_oss(base64_str, user_id="system"):
    ...
    try:
        ...
        # Detect format from data URI (e.g. data:image/webp;base64,...)
        mime = header.split(";")[0].split(":")[1] if ":" in header else "image/webp"
        ext_map = {"image/webp": "webp", "image/jpeg": "jpg", "image/png": "png", "image/gif": "gif"}
    ...
    try:
        ...
        bucket.put_object(filename, file_data, headers={'Cache-Control': 'max-age=2592000', 'Content-Type': mime})
